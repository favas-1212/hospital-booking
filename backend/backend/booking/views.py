...
Features:
  ...
  - [NEW] 2-tokens-ahead email + in-app notification
  - [NEW] OPD pause / resume by doctor (blocks next_token while paused)
  - [NEW] Late-confirmed patient status with queue position & wait estimate
"""
...
# ── [NEW] Shared queue-builder (same logic used by doctor_next_token) ─────────

def _build_ordered_queue(all_qs, opd_day, current_token_num=0):
    """
    Returns the ordered list of WAITING bookings exactly as doctor_next_token
    would call them.  Used for position lookups and the 2-tokens-ahead alert.
    """
...
# ── [NEW] "2 tokens ahead" notification ──────────────────────────────────────

def _notify_two_ahead(remaining_queue):
    """
    Called right after a new patient starts consulting.
    `remaining_queue` = ordered waiting list with the just-called patient
    already removed.

    Index 0 → next to be called (1 ahead).
    Index 1 → the patient we notify  (2 ahead).

    Guards:
      • Only online patients with a registered email.
      • `two_ahead_notified` flag on Booking prevents duplicate emails.
        Add this field to your migration:
            two_ahead_notified = models.BooleanField(default=False)
    """
    if len(remaining_queue) < 2:
        return

    target = remaining_queue[1]   # 2nd in queue → 2 tokens away
    ...
    # Mark to prevent re-sending on the next next_token call
    Booking.objects.filter(pk=target.pk).update(two_ahead_notified=True)


# ── [NEW] Resolve confirmed_type for any booking / opd_day pair ──────────────

def _resolve_confirmed_type(booking, opd_day):
    """
    Returns 'pre_confirmed' | 'on_time' | 'late' | None.
    opd_day may be None if OPD has not started.
    """
...
def patient_token_status(request):
    """
    Returns the patient's today booking with all live status fields:

    [NEW fields]
      opd_paused        – bool: OPD is currently paused
      pause_reason      – str | None: reason given by doctor
      confirmed_type    – 'pre_confirmed' | 'on_time' | 'late' | None
      late_queue_position    – int | None  (only when confirmed_type == 'late')
      late_tokens_ahead      – int | None
      late_estimated_wait_min – int | None
      late_notice       – str | None: human-readable warning for late confirmers
    """
    patient, err = get_patient_or_403(request.user)
    if err:
        return err
    ...
    try:
        ...
        opd_paused   = getattr(opd_day, "is_paused",    False)   # [NEW]
        pause_reason = getattr(opd_day, "pause_reason", None)    # [NEW]
    except OPDDay.DoesNotExist:
        pass

    data = PatientTokenStatusSerializer(booking).data
    data["opd_started"]  = opd_started
    data["opd_paused"]   = opd_paused   # [NEW]
    data["pause_reason"] = pause_reason # [NEW]

    # ── [NEW] confirmed_type ─────────────────────────────────────────────
    confirmed_type = _resolve_confirmed_type(booking, opd_day)
    data["confirmed_type"] = confirmed_type

    # ── [NEW] Late-confirmer queue position & estimated wait ─────────────
...
def patient_confirm_attendance(request, booking_id):
    ...
    3. LATE  (status=WAITING, after 10 min of OPD start)
       ...
       → [NEW] response includes confirmed_type='late' and a warning notice
    """
    patient, err = get_patient_or_403(request.user)
    if err:
        return err
    ...
    response_data = {
        ...
        "confirmed_type": confirmed_type,   # [NEW] always returned
    }

    # [NEW] Extra guidance for late confirmers
    if not is_on_time:
...
def doctor_dashboard(request):
    ...
    for sess in ["morning", "evening"]:
        ...
        opd_sessions_data[sess] = {
            ...
            # [NEW] pause fields visible in doctor dashboard
    ...
    tokens = [{
        ...
        "confirmed_type"      : _resolve_confirmed_type(b, opd_days.get(b.session)),  # [NEW]
        "two_ahead_notified"  : getattr(b, "two_ahead_notified", False),               # [NEW]
    } for b in qs]
...
def doctor_next_token(request):
    ...
    # [NEW] Notify the patient who is now 2nd in the remaining queue
...
def pause_opd(request):
    """
    Pause an active OPD session.

    Body: { "date": "YYYY-MM-DD", "session": "morning"|"evening", "reason": "<optional text>" }

    Effects while paused:
      • doctor_next_token returns HTTP 400 instead of advancing the queue.
      • patient_token_status / queue_status expose opd_paused=True + pause_reason.
      • All WAITING bookings are unaffected; timers simply stop advancing.

    Requires three new fields on OPDDay:
        is_paused    = models.BooleanField(default=False)
        paused_at    = models.DateTimeField(null=True, blank=True)
        pause_reason = models.CharField(max_length=255, blank=True, null=True)
    Add via: python manage.py makemigrations && python manage.py migrate
    """
...
def resume_opd(request):
    ...
    Body: { "date": "YYYY-MM-DD", "session": "morning"|"evening" }
    """
    date_str = request.data.get("date", str(now().date()))
    session  = request.data.get("session")
...
def start_opd(request):
    ...
    Pre-confirmed patients get queue_insert_time=now() → join on-time main queue.
    Only unconfirmed patients receive the start notification email.
    [NEW] pause state is reset to False on every fresh start.
    """
    date_str     = request.data.get("date", str(now().date()))
    session      = request.data.get("session")
...
def queue_status(request):
    ...
    try:
        ...
        return Response({"opd_active": False, "opd_paused": False, "current_token": None, "queue_length": 0})
    ...
    return Response({
        ...
        "started_at"          : opd_day.started_at,
        # [NEW] pause fields exposed to public queue screen
