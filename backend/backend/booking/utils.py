...
def compute_queue_position(booking):
    ...
    try:
        opd_day = OPDDay.objects.get(
            ...
            session=booking.session,   # ✅ was missing session filter
        )
...
def _get_patient_email(booking) -> str | None:
    ...
    Always use this instead of inline checks — it logs exactly
    why an email is missing so you can debug without guessing.
    """
    ...
    # Ensure user is loaded (handles cases where select_related wasn't used)
    try:
...
def send_opd_ticket_email(booking) -> bool:
    ...
    if not email:
        return False   # warning already logged by _get_patient_email
    ...
    try:
        ...
        msg.send(fail_silently=False)   # ✅ raises on SMTP error so logs catch it
